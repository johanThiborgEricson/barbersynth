describe("(new Fraction(n1, d1)).multiply([n2, d2])", function() {
  
  it("calls greatestCommonDivisor twice, first with n1 and d2 and then with " +
  "n2 and d1", function() {
    var fraction = new Fraction("n1", "d1");
  ...
  function() {
    ...
    expect(fraction.multiply([9, 4])).toEqual(new Fraction(3, 2));
  });
  ...
  it("can handle numbers close to one", function() {
    ...
    var fraction = new Fraction(large, notSoLarge);
    expect(fraction.multiply([notSoLarge, large])).toEqual(new Fraction(1, 1));
  });
